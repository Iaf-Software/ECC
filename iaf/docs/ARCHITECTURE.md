...
| `lib/skill-paths.js` | Harness-first discovery | A Cursor reference test resolved a skill through a Claude-oriented path first | IAF | Policy/rules | `iaf/tests/skill-paths.test.js` | Remove rule text |
...
Official ECC ships a project-local Cursor adapter (`.cursor/`) with doctor/install-state. A Cursor marketplace plugin is not the official ECC distribution path. IAF keeps the official adapter so update/repair/doctor remain compatible, including SSH checkouts.
